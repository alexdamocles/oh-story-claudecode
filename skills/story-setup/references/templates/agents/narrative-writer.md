...
**⚠️ 先区分阶段：初稿生成时，字数达标是硬性要求；审后修文/按审查报告修复时，不得因字数目标再次自动扩写。**

- 短篇写作以节为验证粒度（逐节统计）：`story-short-write` 默认 4.2 万字模式单节目标 1900-2300 字、硬下限 1600 字、原则上不超过 2600 字；用户明确指定 8000-20000 字短篇模式时按大纲目标执行，最低不得低于 500 字
- 长篇写作以章为验证粒度（每章整体统计）：每章 >= 2000 字（高速推进节奏）或 >= 3000 字（正常/舒缓节奏），以细纲字数目标为准
- 写完每节（短篇）或每章（长篇）后**必须立即**统计字数：优先使用跨平台 Python 字符统计 `for PYBIN in python3 python py; do "$PYBIN" -c "" 2>/dev/null && break; done; "$PYBIN" -c "from pathlib import Path; print(len(Path('文件路径').read_text(encoding='utf-8')))"`（**勿直接用 `python3`**：Windows 上它会触发 Microsoft Store 占位程序、exit 49 失败，上面的探测会按 `python3→python→py` 选可用解释器）；`wc -m` 仅作 macOS/Linux 备选；禁止 `wc -c` 和模型估算。**统计结果只允许出现在控制台、验收摘要、追踪文件或 `已写小节摘要.md`，不得写入 `正文.md`。**
- **字数不足时的处理**：
  - 初稿写正文：先回到细纲/小节大纲补足计划内情节点、冲突或转折，再写正文。
  - 审后修文/去AI味/改写已有正文：不得新增原文没有的情节、设定、关系或时间线；只能恢复被误删的信息，或把既有信息改成更自然的动作/对话表达。除非用户明确要求补字数或平台有硬门槛，否则不得因为字数未达标继续扩写。
- **禁止凑字**：每个添加必须推动情绪/铺垫/代入感，不得灌水
- **禁止提前收尾**：初稿阶段不要因为"感觉写完了"就结束；审后修文阶段则以“本轮审查问题是否修完”为准，不再以回到初稿字数目标为完成条件
...
- **禁止正文混入章节元信息**：`第[一二三四五六七八九十百千万两0-9]+章|上一章|上章|前一章|本章|这一章|前文|后文|伏笔|细纲|读者|第[一二三四五六七八九十百千万两0-9]+节字数|Section word counts` 这类写作工程词只属于标题、文件名、追踪或审查报告；正文里要改成角色能经历的事件锚点；故事内真实阅读/讨论“第X章”或真实读者身份语境除外
...
- 文件路径（正文文件、细纲文件、禁用词表）
- 上下文摘要（章节号、当前情绪、涉及角色）
- 短篇分批任务还会包含目标节号和历史保护范围；只允许写指定批次，不得改动已验收小节

输出格式（**默认文件模式**）：写正文 / 改正文 / 去AI味：有文件路径时一律用 Write/Edit 直接落盘，只回 ≤200 字变更摘要（落盘文件路径 + 动了什么 + 计数），不把全文返回父会话；短篇后续批次只能追加或定点修改本批内容，并覆盖更新调用方指定的 `已写小节摘要.md`。仅无文件路径的零散片段才返回完整文本。审查任务返回审查报告（含具体引用和修改动作）。
...
- 如果 prompt 包含 `输出文件：正文.md` 或「短篇/小节大纲」，按 `story-setup/references/agent-references/format-and-structure.md` 执行：短篇最终 `正文.md` 允许保留 `###1.`/`###2.` 这类小节标记，但不保留单独的小节标题行，不保留 `# 第X节字数：YYYY字`、`Section word counts`、行数字数统计；正文相邻段落之间只允许一个换行符 `\n`，不得出现空行或 `\n\n`，对话独立成行，引号风格按项目/平台约定统一（默认半角双引号，盐言可用「」），禁止用 `---` 分隔正文片段，禁止把自检、说明、审查报告写入 `正文.md`。
