...
> **配合**：女频复仇事件与情绪公式见 female-revenge-writing-formulas.md，读者画像见 genre-readers.md。
...
**矛盾加码（时间轴上一层层升级）**：上面的冲突网络是同时运作的"面"，这里是时间线上的"加码"，越往后越往痛处走。短篇世情尤其吃这套。
...
锚到现实痛点（家庭/职场/婚恋）才有共鸣；推进靠具体行动（取证、走法律、处置财产），不靠对骂。爽点的强弱递增是另一根轴，见 `emotional-arc-design.md`。
...
实例：重生+悬疑融合，用一句设定圈住——女主重生后发现每次预知都来自凶手留的线索，于是预知(重生)和查案(悬疑)共用同一根线，不是两张皮。先找到能同时解释两元素的设定再动笔。
...
基调=读者点开期待的那种爽法。检查法：随机抽中段一节，问它给的情绪和开头承诺的是不是同一种(追妻虐恋里突然插一段轻松沙雕=基调崩)。爽文别在打脸高潮处转抒情慢节奏。
...
**友情vs爱情**：恋爱写成持续的小磨合再让步：为谁洗碗、过年回谁家、要不要孩子——这类鸡毛蒜皮的拌嘴+和好，比一味甜更像真情侣。每隔几节安排一次小分歧再化解，别一路顺甜。
