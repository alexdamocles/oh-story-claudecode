...
默认段落交付排版是**按戏剧单元/镜头自然断段，段落紧密排列**。不要把**段落字数**当成强制切刀；先判断“一件事/一个推理链/一个情绪变化”是否完成。但每小节仍必须按既定情节任务完成，并遵守本技能的单节与全书字数硬约束。
...
- **默认 4.2 万字模式**：默认 20 节，情节确有需要时可在 18-22 节内调整；单节目标 1900-2300 字，硬下限 1600 字，原则上不超过 2600 字
- 本技能固定使用 4.2 万字模式：初稿 40000-45000 字、默认 20 节、单节目标 1900-2300 字，不得切换短篇模式。
- 每小节推进一个明确的情节点
...
1. **一个主事件** + **3-5 个子事件**（主事件推进核心情节，子事件丰富层次；初稿偏短时先回到小节/细纲补足计划内子事件、对话或冲突，再写正文；审后修文/去AI味已有正文时不得为补字数新增剧情，除非用户明确要求扩写或平台有硬门槛）
2. **一个情绪变化**（读者的感受如何变化）
3. **一条读者新获知的信息**
