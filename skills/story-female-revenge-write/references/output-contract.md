...
description: |
  ...
  下游消费规范（story-female-revenge-write 读全套 markdown + 原文 + _meta.json 写新短篇）。
sync-source: skills/story-short-analyze/references/output-contract.md
sync-policy: |
  本文件在 story-short-analyze 与 story-female-revenge-write 之间需保持字节一致（byte-equal）。
  修改任一副本后，必须同步另一副本，并通过 bash scripts/check-shared-files.sh 验证。
  禁止把本文件加入 IGNORE_NAMES 列表——它必须保持同步，不属于 intentional differences。
---

# 输出契约：story-short-analyze ↔ story-female-revenge-write

`story-short-analyze` 拆完一篇短篇后，产物落盘到 `拆文库/{书名}/`。`story-female-revenge-write`
...
**文件名约定**：`拆文报告.md / 情节节点.md / 写作手法.md` 由 `story-female-revenge-write` 硬编码
...
用户「拆解完成，可调用 `/story-female-revenge-write` 写下一篇」。

---

## 下游消费规范（story-female-revenge-write 怎么用）

> `story-female-revenge-write` 当前硬编码读 `拆文报告.md / 情节节点.md / 写作手法.md` 三个 markdown。
...
/story-female-revenge-write 拆文库/{书名}/
