...
**本文件职责**（其他内容去哪看）：
- 女频复仇常见反派套路（重生复仇反派、身份压制反派等）见 genre-writing-formulas.md，本文件只提供通用设计模板
- 本文件的反派人设模板 `{身份}+{动机}+{作恶方式}+{致命弱点}` 侧重「设计蓝图」，genre-writing-formulas.md 的 `{人设}+{动机}+{行为}+{态度}+{受伤害程度}` 侧重「矛盾冲突公式」——前者用于 Phase 2 设计，后者用于 Phase 4 写作
