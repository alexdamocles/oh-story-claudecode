...
当用户触发 `/story-female-revenge-write` 或提出女频虐渣、古言复仇、追妻火葬场故事时，**一律先进入 Phase 1**。启动段不判定自动决策、不生成候选、不落盘 `设定.md`，也不生成小节大纲或正文。

外部编排器或用户若明确给出现有小说工程路径并要求“续写/继续写/根据审查报告修文/修复这篇”，由 Phase 1 识别并进入指定目录；不得自行扫描 novels 目录、创意索引或数据库来决定哪本未完成。
...
7. 最终选定候选后，不得写入、追加、创建或重写 `创意索引.jsonl`。`创意索引.jsonl` 只作为只读去重输入；创意入库、更新和删除统一由外部编排器/uploader 服务层维护。设定没写完不算创意完成，但本 skill 只负责把完整设定写入 `设定.md`，不得直接更新索引文件。

### 创意索引只读边界

`创意索引.jsonl` 只作为选题去重输入。本 skill 不负责维护创意库，不得创建、追加、修改、删除或重写 `创意索引.jsonl`。

1. `设定.md` 必须已经落盘，并包含：标题、目标字数、情绪目标、一句话梗概、背叛类型/等级、反杀方式/层级、男主下场、核心反转、人设速写、故事发动机、资源与限制、对抗升级链、配角功能、证据与伏笔链、关系阶段变化、自动决策记录。
2. 上述字段缺一项，就继续补 `设定.md`；不要把半截设定、候选方案或失败中断想法写入 `创意索引.jsonl`。
...
2. 用户明确要求“先问我”“让我选”“先给我几个方案”或“大纲确认后再写”时：停在当前阶段，等待用户的明确指令。
3. 其余情况，包括只触发技能、只给模糊方向、未提供现有工程路径：进入本阶段的自动决策分支，按自动新开一篇处理。

#### 自动决策分支（默认）

除非命中上述现有工程或人工确认条件，否则不得追问用户方向；直接在 `novels/{短篇标题}/` 下创建本篇工程。自动分支不在 Phase 1 落盘文件，而是依次进入 Phase 2 和 Phase 3：

1. 不在选题、情绪目标、核心框架、小节大纲阶段等待用户选择或确认。
2. 用户给出模糊方向时，将其作为 Phase 2 的约束条件；在约束内自动选择最优方案，不把候选列表交给用户投票。
3. Phase 3 的大纲自检通过后直接进入 Phase 4 正文写作，不等待人工确认。
...
默认写约 42000 字（40000-45000 字）古代或民国背景女频虐渣爽文时，必须把三份 md 当作补充参考库：

1. 先读 `references/corpus-19-structure-profile.md`，用于故事发动机、资源代价、对手反制、20 节事件骨架和新秩序攻防；不重新扫描原始 ASR 语料。
2. 再读 `references/female-revenge-longform-pipeline.md` 的相关小节，用于选题矩阵、背叛等级、反杀层级、男主下场、5 段式字数分配、暴力镜头距离。
3. 只在需要快速生成钩子/硬技能/爽点组合时，读 `references/female-short-pipeline-formula.md` 的相关小节；它提供组合，不决定最终主线。
4. 冲突优先级：`SKILL.md` 硬约束 > `format-and-structure.md`/`anti-ai-writing.md` > `corpus-19-structure-profile.md` > `female-revenge-longform-pipeline.md` > `female-short-pipeline-formula.md`。
5. 三份补充参考库不得替代对标拆文、正文格式、字数验证和去 AI 味流程；只模仿结构习惯，不复制语料原句、人物、专有设定或完整情节组合。
...
> **拆文产出格式**：analyze 落盘的完整文件树、`_meta.json` schema、Stage→文件映射，以及「story-female-revenge-write 怎么读这些产出」的下游消费规范，见 [references/output-contract.md](references/output-contract.md)。
