...
description: |
  ...
  触发方式：/story-short-write、/写短篇、「帮我写一篇短篇」「写个盐言故事」。用户只触发命令但不给方向时，直接按自动决策模式新开一篇；是否续写旧工程、派发哪本书，由外部编排器或用户明确路径决定。
metadata:
  openclaw:
...
当用户触发 `/story-short-write`、`/写短篇` 或只说“写一篇短篇/盐言故事”但没有提供题材、梗概、参考书、现有小说目录时，不要回复“你想写什么”或要求用户给方向。把空请求视为“自动新开一篇”，立即执行：
...
正文写作阶段默认按 2-3 节/批推进，**统一由主会话直接写正文**，不再拉起 `narrative-writer` 或其他子 agent 代写当前批次。主会话负责确定本批要写的 2-3 节、读取 `小节大纲.md`、组装续写上下文、直接写入 `正文.md`、覆盖更新 `已写小节摘要.md`，并对本批产物做格式与质量验收。不要试图把 40000+ 字全文一次性写完，仍按批次串行推进。

每批写完后必须覆盖更新 `novels/{短篇标题}/已写小节摘要.md`（3-5 条：已揭示信息、情绪位置、未回收伏笔、下一批衔接句）。下一批开始前，主会话必须先读取 `已写小节摘要.md` 和 `正文.md` 尾部 300-500 字，再继续当前批次。最终写入 `正文.md` 前必须按同一格式规范重排一次，保证落盘正文始终是统一标准形态。
⚠️ **阶段分流必须先判定**：
...
写完每节后必须统计字数和行数。**仅在初稿生成阶段**，不足 800 字（短篇例外模式不足 500 字）的节不得跳过，必须补充更多子事件/对话来补足后再写下一节。初稿完成后总字数目标为 40000-45000 字；超过 45000 字时必须压缩冗余段落，除非用户明确要求更长。审后修复阶段只记录实际字数，不因低于 30000 字或未回到初稿目标自动扩写。
...
如果任何一节 < 800 字（用户明确切回短篇例外模式时 < 500 字）→ 仅初稿阶段补充更多子事件/对话来补足后再写下一批；审后修复阶段不得仅因这里未达标就继续扩写。
禁止跳过未达标的小节。
...
- [ ] 初稿阶段：每节 ≥ 800 字（用户明确切回短篇模式时，才可按题材降至 ≥500 字）
- [ ] 节数 = 小节大纲规划节数（不得合并/省略）
- [ ] 身体部位同一词全文 ≤ 5 次
- [ ] 「像」≤ 10 处
