...
1. 不扫描 `novels/` 下的未完成工程，不自动判断续写还是新开。
2. 直接在 `novels/{短篇标题}/` 新建本篇工程，并读取默认参考库和项目根目录 `创意索引.jsonl`。
3. 自动生成 3 个候选选题并评分去重。
4. 自动选定最优题材、核心框架和不超过 24 节的大纲。
5. 大纲自检通过后直接开始写 `正文.md`。

只有用户明确给出现有小说工程路径，并明确要求“续写/继续写/根据审查报告修文/修复这篇”时，才进入指定目录处理；不得自行从 `novels/` 扫描候选目录，也不得因为发现旧稿而改变本次新开流程。
...
除非用户明确要求“让我选”“先给我几个方案”“大纲确认后再写”，否则全流程默认自动推进。用户没有给题材方向时，不得追问；不扫描 `novels/` 下的未完成工程，直接把本次任务按“自动新开一篇”处理，并在 `novels/{短篇标题}/` 下创建本篇工程：

1. 不在选题、情绪目标、核心框架、小节大纲阶段等待用户选择或确认；启动时不执行未完成工程扫描，生成目录固定为 `novels/{短篇标题}/`。
