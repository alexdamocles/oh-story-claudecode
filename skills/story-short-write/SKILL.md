...
详细规则见 `references/format-and-structure.md`，写作前必须加载。**主会话与 narrative-writer 子代理使用同一套正文格式**：正文只允许保存在 `正文.md`，正文相邻段落之间只允许一个换行符 `\n`（不得出现空行/`\n\n`），对话引号风格按项目/平台约定统一（默认半角双引号，盐言可用「」）。**短篇 `正文.md` 最终交付形态为纯正文**：不保留 `###1.`/`###2.` 这类小节标记，不保留单独的小节标题行，不保留 `# 第X节字数：YYYY字`、`Section word counts`、行数字数统计、自检说明或任何写作工程元信息；节次只用于内部规划、批次控制和验收，不进入最终正文文件。如果子代理输出与主会话格式不一致，按本格式规范重排后再写入文件。
...
正文写作阶段默认按 2-3 节/批推进，但正文执行者改为 narrative-writer 优先。主会话先检查 `.claude/agents/` 下的 `narrative-writer.md` 是否存在；不存在时再检查 `.opencode/agents/`。存在时，默认由 `narrative-writer` 写当前批次正文；主会话只负责确定本批要写的 2-3 节、组装 prompt、验收 `正文.md` 与 `已写小节摘要.md`，并在不合格时直接发修正文指令。不要要求单次 agent spawn 完成 40000+ 字全文。每批写完后必须覆盖更新 `novels/{短篇标题}/已写小节摘要.md`（3-5 条：已揭示信息、情绪位置、未回收伏笔、下一批衔接句）；无论摘要由谁先产出，主会话都必须复核后再进入下一批。下一批必须先读取 `已写小节摘要.md` 和 `正文.md` 尾部 300-500 字再续写。只有在 `narrative-writer` 不存在、当前会话无法调用该 subagent，或用户明确要求主会话直接写正文时，才回退为主会话直写。存在时 spawn `Agent(subagent_type: "narrative-writer", prompt: "项目目录：novels/{短篇标题}\n任务描述：写正文\n输出文件：novels/{短篇标题}/正文.md\n情绪目标：{从核心框架读取}\n小节大纲：novels/{短篇标题}/小节大纲.md（读取结构段/五段功能、预计字数、人物/关系变化、因果/逻辑链、结尾承接/钩子；默认使用约 42000 字（40000-45000 字）五段式蓝图）\n涉及角色：{从核心框架读取}\n主对标/拆文路径：{本次查找到的主对标 novels/{短篇标题}/对标/{主对标书}/ 或 拆文库/{主对标书}/，没有则写 无}\n主拆文召回摘要：{本场景最相关的主对标结构/情绪/反转/写作手法模块；按场景相关性压缩，不写固定5条上限；没有则写 无}\n副对标召回摘要：{按 references/cross-book-recall.md 阶段预算筛选后的副对标/参考对标结构化摘要表；可含多本，但只传摘要，不传副书文风/原文；没有则写 无}\n续写上下文：必须先读取 `novels/{短篇标题}/已写小节摘要.md` 与 `novels/{短篇标题}/正文.md` 尾部 300-500 字，再继续当前批次。\n交付要求：写完当前批次正文后，同时覆盖更新 `novels/{短篇标题}/已写小节摘要.md`，供主会话验收与下一批续写使用。字数统计结果只允许出现在控制台、验收摘要或 `已写小节摘要.md`，不得写入 `正文.md`。短篇 `正文.md` 只保留纯正文，不保留 `###7.`、`边关烽火` 这类单独小节标题行，不保留 `# 第7节字数：2973字` 这类统计行。\n格式硬约束：必须完全遵守 story-short-write/references/format-and-structure.md；短篇最终正文不保留小节标记、不保留单独小节标题行；正文相邻段落之间只允许一个换行符 `\n`，不得出现空行/`\n\n`；对话独立成行，引号风格按项目/平台约定统一（默认半角双引号，盐言可用「」）；禁止使用 --- 分隔正文片段；禁止把自检/说明/审查报告、字数统计、节次编号写入正文.md。\n写作硬约束：禁止先否定再肯定的翻转句式，含省略连接词、跨句或换行变体；按三维度揉进写场景，但仍必须按戏剧单元/画面分段；一段承载一个完整动作-信息变化或一条连续推理/氛围/情绪链，不按固定字数强拆。输出前做自然节奏重排：场景/一件事结束才分段；新动作、新线索、新对话、视线切换另起；完整推理、氛围铺陈、情绪变化可保留稍长段。高潮/打脸/反转压短，沉淀/推理/收束允许长一点，爽点 beat 写密、过场 beat 写疏，忌通篇同长度或同一阈值切段（见 writing-craft.md「疏密分配」）。主语节奏：段首或主语重置时可用主角名；同一动作链内优先代词/省略；关键转折再点名强调，避免连续句/段无必要重复主角名。标点节奏：按语气标点谱系执行，避免通篇句号化，也禁止随机堆砌问号/感叹号；质问用问号，爆发处少量感叹；犹豫、未尽、打断或拖长用动作停顿、短句、换行处理，正文产物不使用 `……` / `——` / `—` / `--`。具体字数表达校验：评价台词、题字、信件、念头或弹幕时，只有统计口径明确、已用脚本逐字核对且故事确有必要，才使用“这五个字 / 短短四字 / 三个字一落 / 八个字砸下去”这类具体字数表达；不能确保字数计算正确时，改成“这句话一落”“这一句落下”“那几个字”“这行字”“话音落下”等非具体数字表达。叙述姿态锁深度限知此刻感知，不跳出解释因果/不剧透预告/不替读者总结升华（去说教·上帝感·安排感，见 anti-ai 模式8/Gate G）；情绪宁烈不温，冲突前置、爽点要狠要具体、台词带刺，敢写极端反应不点到为止（以克制为爽感的题材如虐文/世情除外，按 genre-catalog 走克制路线）。")`。无论由谁写作，最终写入 `正文.md` 前都必须按同一格式规范重排一次；在 agent 优先模式下，主会话负责最终验收与回退修正，保证落盘正文始终是统一标准形态。
