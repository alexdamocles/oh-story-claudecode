...
version: 1.0.0
description: |
  短篇/中篇网文写作。辅助女频爽文、虐渣爽文、盐言故事等从构思到成稿；默认生成 30000-40000 字女频虐渣爽文，聚焦情绪拉扯、反杀节奏与爽感释放。
...
你是短篇/中篇网文写作执行器。从构思到成稿，完成一篇完整小说。默认目标是 30000-40000 字古代优先、少量民国的女频虐渣爽文；只有用户明确要求更短时，才切回 8000-20000 字短篇模式。
...
3. `设定.md` 和 `小节大纲.md` 完整，但 `正文.md` 缺失、总字数 < 30000、或正文小节数少于大纲规划小节数 → 继续写 `正文.md`。
4. 同时发现多个未完成工程时，自动选择最近修改的一个继续；在回复里用一句话说明选择了哪个目录。
...
6. **默认 30000-40000 字古代优先、少量民国背景**。按 5 段式写完整古代或民国背景女频虐渣爽文：伤害 10%、隐忍 20%、反杀 30%、上升 30%、火葬场 10%。用户没有特别指定时，不按 8000 字短篇收束。
...
- **默认加载女频虐渣参考库**：默认 30000-40000 字古代或民国背景女频虐渣爽文时，先用 `female-revenge-longform-pipeline.md` 做选题矩阵、5 段式结构、背叛/反杀/火葬场校准；需要快速生成钩子、硬技能、爽点组合时，再用 `female-short-pipeline-formula.md` 补充
...
2. 情绪目标固定为“意难平 + 爽感释放”，背景默认固定为古代优先、少量民国，不再生成情绪候选、不再询问用户选择情绪。用户没有明确方向时，先读取 `创意索引.jsonl` 做去重，再围绕“古代/民国背景 + 固定情绪”生成 3 个候选选题，用“背叛烈度、意难平强度、爽感兑现度、反杀层级匹配度、时代背景落地度、3-4 万字支撑力、写作新鲜感”七项各 1-10 分打分，自动选择总分最高且 30000-40000 字最可落地的一项。背景选择按长期配比控制：默认约 5 篇里 4 篇古代、1 篇民国；如果没有历史记录，当前批次优先先出古代。
3. 用户给出模糊方向时，将其视为约束条件，在约束内生成候选并自动选最优，不把候选列表交给用户投票。
4. 小节大纲写完后自行做反转信息差验证、伏笔回查、5 段式比例与 30000-40000 字可行性检查；检查通过就进入正文写作，不等待人工确认。
...
如果用户只有模糊想法或没有给方向 → 不再询问用户选择，按“自动决策模式”在“意难平 + 爽感释放 + 古代/民国背景”内自行确定题材方向。默认只生成古代宫廷、侯门、宅斗、替嫁、和离、流放、重生复仇、权谋婚姻，以及少量民国豪门、军阀、姨太太、戏班、乱世婚姻等框架；古代背景有默认加成，做到长期约 5 篇里 1 篇民国，其余优先古代；绝对不能生成现代、都市、校园背景。优先选择先压出遗憾、误解、亏欠或错失，再通过反转、打脸、复仇、醒悟、补偿或自我成全完成释放的组合；不要为了猎奇选择难以在 30000-40000 字内完成情绪闭环的设定。
...
默认写 30000-40000 字古代或民国背景女频虐渣爽文时，必须把两份新增 md 当作补充参考库：
...
构思阶段，如果项目已部署 story-architect agent（优先检查 `.claude/agents/` 下的 `story-architect.md` 是否存在；不存在时再检查 `.opencode/agents/`），可 spawn `Agent(subagent_type: "story-architect", prompt: "项目目录：novels/{短篇标题}\n任务类型：30000-40000 字古代或民国背景女频虐渣爽文构思\n模式：自动决策，不等待用户选择\n查询参数：{固定情绪目标：意难平 + 爽感释放；固定背景：古代优先、民国约占五分之一，绝不现代；默认目标字数：30000-40000；题材方向}\n要求：先按需读取 `references/female-revenge-longform-pipeline.md` 做选题矩阵、背叛/反杀/火葬场校准；再读取 `创意索引.jsonl` 做六维去重；生成不少于3个候选核心框架，候选必须与最近历史框架至少3个维度不同；按背叛烈度、意难平强度、爽感兑现度、反杀层级匹配度、3-4 万字支撑力、写作新鲜感六项评分，并执行重复惩罚；选择总分最高且最可落地的一项，输出最终框架和自动决策记录。")` 辅助框架设计。如 agent 不可用，由主线程直接执行同一套自动决策流程。
...
3. 确定揭露方式 → 同上
4. 编写 小节大纲.md（格式见 writing-workflow.md）：默认按 30000-40000 字规划不超过 24 节（建议 18-24 节，单节约 1300-2000 字），每节标明结构段/五段功能、预计字数、人物/关系变化、因果/逻辑链、结尾承接/钩子
...
写完每节后必须统计字数和行数。不足 800 字（短篇例外模式不足 500 字）的节不得跳过，必须补充更多子事件/对话来补足后再写下一节。整篇完成后总字数必须 ≥ 30000 字；超过 40000 字时必须压缩冗余段落，除非用户明确要求更长。
...
- [ ] 总字数 30000-40000（优先用 Python 字符统计验证，兼容 Windows 和中文字符计数；用户明确短篇模式除外）
...
**流水线：** 短篇/中篇（默认 30000-40000 字古代优先、少量民国背景女频虐渣爽文）
**位置：** 写作（第 3/3 步）
...
| [references/female-revenge-longform-pipeline.md](references/female-revenge-longform-pipeline.md) | 默认 30000-40000 字古代或民国背景女频虐渣爽文时；按需读取选题矩阵、背叛/反杀层级、5 段式结构、火葬场 |
...
| 古代/民国女频虐渣爽文 | **`references/female-revenge-longform-pipeline.md`**（30000-40000 字古代/民国选题矩阵 / 5 段式 / 背叛反杀匹配 / 火葬场） | `references/female-short-pipeline-formula.md`（快速钩子与爽点组合）· `references/female-audience-writing.md` |
