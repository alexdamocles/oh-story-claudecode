...
1. 先扫描当前工作区是否有未完成小说工程
2. 有未完成工程则自动续写，不新开题
3. 没有未完成工程时，读取默认参考库和 `创意索引.jsonl`
4. 自动生成 3 个候选选题并评分去重
...
每次触发本 skill 时，先扫描当前项目根目录和一级子目录，寻找未完成小说工程；续写优先级高于自动新开题。用户明确说“新开一篇”“不要续写”“忽略旧稿”时才跳过扫描结果。
...
除非用户明确要求“让我选”“先给我几个方案”“大纲确认后再写”，否则全流程默认自动推进。用户没有给题材方向时，不得追问；先扫描未完成工程，有未完成则续写；没有未完成时才把本次任务按“自动来一个”处理：

1. 不在选题、情绪目标、核心框架、小节大纲阶段等待用户选择或确认；但启动时必须先执行“未完成工程续写优先级”。
...
框架自动确定后，完成设计任务，然后在工作目录下创建文件。不要停下来询问用户是否接受框架；只有用户明确要求人工确认时才暂停。
...
设计任务完成后，如果项目已部署 character-designer agent（优先检查 `.claude/agents/` 下的 `character-designer.md` 是否存在；不存在时再检查 `.opencode/agents/`），可 spawn `Agent(subagent_type: "character-designer", prompt: "项目目录：{dir}\n任务类型：角色设定\n查询参数：{人设速写+关系}")` 辅助角色设定和语言风格档案。如 agent 不可用，由主线程直接执行。
