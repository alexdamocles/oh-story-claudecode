...
当用户触发 `/story-short-write`、`/写短篇` 或只说“写一篇短篇/盐言故事”但没有提供题材、梗概、参考书时，不要回复“你想写什么”或要求用户给方向。把空请求视为“自动来一个”，立即执行：

1. 先扫描当前工作区的 `novels/` 是否有未完成小说工程
2. 有未完成工程则自动续写，不新开题
3. 没有未完成工程时，在 `novels/{短篇标题}/` 新建本篇工程，并读取默认参考库和项目根目录 `创意索引.jsonl`
4. 自动生成 3 个候选选题并评分去重
5. 自动选定最优题材、核心框架和不超过 24 节的大纲
6. 大纲自检通过后直接开始写 `正文.md`
...
每次触发本 skill 时，先扫描当前项目根目录下的 `novels/` 及其一级子目录，寻找未完成小说工程；续写优先级高于自动新开题。用户明确说“新开一篇”“不要续写”“忽略旧稿”时才跳过扫描结果。默认不要在工作区根目录创建或续写新小说工程；只有用户明确给出根目录旧稿路径时，才兼容读取根目录旧结构。

未完成工程判定：

0. **完结标记优先**：小说目录内只要存在 `最终审查报告.md`，即判定为已完结，必须从未完成工程候选中排除；不得因为 `正文.md` 字数低于 30000/40000、审后净稿变短、或初稿目标未达标而再次拉出续写/重写。若只有 `审查报告_*.md` 且没有 `最终审查报告.md`，说明仍在审后修复或复审阶段，按下面规则继续判断。
1. `设定.md` 存在但缺少标题、目标字数、情绪目标、一句话梗概、背叛类型/等级、反杀方式/层级、男主下场、核心反转、人设速写任一关键字段 → 设定未完成，先补完 `设定.md`，不写正文，不写创意索引。
2. `设定.md` 完整，`小节大纲.md` 缺失或未列出不超过 24 节的规划 → 继续完成 `小节大纲.md`。
3. `设定.md` 和 `小节大纲.md` 完整，且目录内没有 `最终审查报告.md` 时，只有 `正文.md` 缺失、或正文小节数少于大纲规划小节数，才继续写 `正文.md`；不要把“总字数低于 40000/30000”单独作为未完成依据。
4. 同时发现多个未完成工程时，先排除已有 `最终审查报告.md` 的目录，再自动选择最近修改的一个继续；在回复里用一句话说明选择了哪个目录。

审后修复阶段例外：如果当前小说目录已经存在 `审查报告_*.md` 或 `最终审查报告.md`，且本轮任务是根据审查意见修文，则修完 S1/S2/S3 问题后只核对结构闭环、计划小节是否完成、因果和情绪是否成立；不得仅因为正文净稿低于 30000 字自动扩写。只有用户明确要求“扩写/补到 3 万/补到指定字数”，或大纲计划小节尚未写完时，才继续新增正文。

续写正文时必须读取 `设定.md`、`小节大纲.md`、`正文.md` 尾部 300-500 字和最近的“已写小节摘要”（如有），从下一个未完成小节继续，不重写已完成正文。只有发现设定或大纲缺失/冲突时，才回退补设定或大纲。
...
除非用户明确要求“让我选”“先给我几个方案”“大纲确认后再写”，否则全流程默认自动推进。用户没有给题材方向时，不得追问；先扫描 `novels/` 下的未完成工程，有未完成则续写；没有未完成时才把本次任务按“自动来一个”处理，并在 `novels/{短篇标题}/` 下创建本篇工程：

1. 不在选题、情绪目标、核心框架、小节大纲阶段等待用户选择或确认；但启动时必须先执行“未完成工程续写优先级”，默认扫描和生成目录均为 `novels/{短篇标题}/`。
...
4. 续写旧工程时，如果 `设定.md` 已完整但索引缺失，应先补一行索引，再继续大纲或正文。
