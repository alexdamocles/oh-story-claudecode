...
1. 读取默认参考库和 `创意索引.jsonl`
2. 自动生成 3 个候选选题并评分去重
3. 自动选定最优题材、核心框架和 35-45 节大纲
...
除非用户明确要求“让我选”“先给我几个方案”“大纲确认后再写”，否则全流程默认自动推进。用户没有给题材方向时，不得追问；直接把本次任务按“自动来一个”处理：
...
7. 最终选定后，把结构化一行追加到 `创意索引.jsonl`，并可选追加一段人类可读摘要到 `创意历史.md`。索引行必须足够短，不写正文、不写长大纲。
