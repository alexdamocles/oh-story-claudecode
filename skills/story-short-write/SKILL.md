...
详细规则见 `references/format-and-structure.md`，写作前必须加载。**主会话与 narrative-writer 子代理使用同一套正文格式**：正文只允许保存在 `正文.md`，正文相邻段落之间只允许一个换行符 `\n`（不得出现空行/`\n\n`），对话引号风格按项目/平台约定统一（默认半角双引号，盐言可用「」）。**短篇 `正文.md` 最终交付形态允许保留 `###1.`/`###2.` 这类小节标记**，但不保留单独的小节标题行，不保留 `# 第X节字数：YYYY字`、`Section word counts`、行数字数统计、自检说明或任何写作工程元信息；节次只用于内部规划、批次控制和验收。如果子代理输出与主会话格式不一致，按本格式规范重排后再写入文件。
